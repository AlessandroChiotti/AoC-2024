...
func computeScore(currPosition position, prevValue int, topographicMap matrix, trailEndPositions map[position]struct{}) {
	x := currPosition.x
	y := currPosition.y
	...
	if topographicMap.data[y][x] == 9 {
		trailEndPositions[currPosition] = struct{}{}
	}

	computeScore(position{x: x + 1, y: y}, prevValue+1, topographicMap, trailEndPositions)
	computeScore(position{x: x - 1, y: y}, prevValue+1, topographicMap, trailEndPositions)
	computeScore(position{x: x, y: y + 1}, prevValue+1, topographicMap, trailEndPositions)
	computeScore(position{x: x, y: y - 1}, prevValue+1, topographicMap, trailEndPositions)
...
func computeResult(topographicMap matrix, trailheadPositions []position) int {
	...
	for _, trailheadPosition := range trailheadPositions {
		trailEndPositions := make(map[position]struct{})
		computeScore(trailheadPosition, -1, topographicMap, trailEndPositions)
